...
exposure_file <- "/Users/sanjeedahs/Desktop/NEWMR/HF/HF.tsv"
...
# Perform LD clumping
HF_clump <- clump_data(exposure_data, clump_r2 = 0.01, pop = "EUR")
...
```{r}
# Harmonise the data
HF_harmonised_data <- harmonise_data(HF_clump, outcome_data)

# Check the number of SNPs after clumping and harmonization
print(paste("Number of SNPs after clumping:", nrow(HF_clump)))
print(paste("Number of SNPs after harmonization:", nrow(HF_harmonised_data)))
...
HF_mr_results <- mr(HF_harmonised_data)
...
HF_heterogeneity_results <- mr_heterogeneity(HF_harmonised_data)
```
...
```{r}
# Test for pleiotropy
HF_pleiotropy_results <- mr_pleiotropy_test(HF_harmonised_data)
...
HF_mr_results <- HF_mr_results %>%
  mutate(lower_ci = b - 1.96 * se,
         upper_ci = b + 1.96 * se)

```

9. View the results for univariable MR, heterogeneity and pleiotropy
...
print(HF_heterogeneity_results)

print("Pleiotropy Test Results:")
print(HF_pleiotropy_results)
...
11. MR IVW analysis using the 'MendelianRandomization' package
...
mrdata <- mr_input(
  bx = HF_harmonised_data$beta.exposure,
  bxse = HF_harmonised_data$se.exposure,
  by = HF_harmonised_data$beta.outcome,
  byse = HF_harmonised_data$se.outcome,
  snps = HF_harmonised_data$SNP
)
...
loo_results <- mr_leaveoneout(HF_harmonised_data)
...
HF_harmonised_data <- HF_harmonised_data %>%
  mutate(lower_ci = beta.outcome - 1.96 * se.outcome,
         upper_ci = beta.outcome + 1.96 * se.outcome)

# Create forest plot
forest_plot <- ggplot(HF_harmonised_data, aes(y = reorder(SNP, beta.outcome), x = beta.outcome)) +
  geom_point() +
  geom_errorbarh(aes(xmin = lower_ci, xmax = upper_ci), height = 0.2) +
  labs(x = "MR effect size for 'HF' on 'AF' (Beta with 95% CI)", y = "SNP", title = "Forest Plot of MR Results for Individual SNPs for HF and AF") +
  theme_minimal() +
...
funnel_plot <- ggplot(HF_harmonised_data, aes(x = beta.outcome, y = 1 / se.outcome)) +
  geom_point(alpha = 0.5, color = "blue") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs(x = expression("Genetic association with outcome " ~ beta[IV]), 
       y = expression("Inverse Standard Error " ~ 1 / SE[IV]), 
       title = "Funnel Plot to Inspect Horizontal Pleiotropy for HF and AF") +
  theme_minimal() +
...
scatter_plot <- mr_scatter_plot(HF_mr_results, HF_harmonised_data)
...
head(HF_harmonised_data)

# Save the harmonized data to a CSV file in the specified directory
output_file <- "/Users/sanjeedahs/Desktop/NEWMR/HF/HF_harmonized_SNPs.csv"
write.csv(HF_harmonised_data, output_file, row.names = FALSE)
