...
exposure_file <- "/Users/sanjeedahs/Desktop/NEWMR/SBP/SBP_38.tsv"
...
# Perform LD clumping
SBP_clump <- clump_data(exposure_data, clump_r2 = 0.01, pop = "EUR")
...
```{r}
# Harmonise the data
SBP_harmonised_data <- harmonise_data(SBP_clump, outcome_data)
...
SBP_mr_results <- mr(SBP_harmonised_data)
...
SBP_heterogeneity_results <- mr_heterogeneity(SBP_harmonised_data)
```
...
# Test for pleiotropy
SBP_pleiotropy_results <- mr_pleiotropy_test(SBP_harmonised_data)
...
print(SBP_mr_results)

print("Heterogeneity Test Results:")
print(SBP_heterogeneity_results)

print("Pleiotropy Test Results:")
print(SBP_pleiotropy_results)
...
mrdata <- mr_input(
  bx = SBP_harmonised_data$beta.exposure,
  bxse = SBP_harmonised_data$se.exposure,
  by = SBP_harmonised_data$beta.outcome,
  byse = SBP_harmonised_data$se.outcome,
  snps = SBP_harmonised_data$SNP
)
...
loo_results <- mr_leaveoneout(SBP_harmonised_data)
...
SBP_harmonised_data <- SBP_harmonised_data %>%
  mutate(lower_ci = beta.outcome - 1.96 * se.outcome,
         upper_ci = beta.outcome + 1.96 * se.outcome)

# Create forest plot
forest_plot <- ggplot(SBP_harmonised_data, aes(y = reorder(SNP, beta.outcome), x = beta.outcome)) +
  geom_point() +
  geom_errorbarh(aes(xmin = lower_ci, xmax = upper_ci), height = 0.2) +
  labs(x = "MR effect size for 'SBP' on 'AF' (Beta with 95% CI)", y = "SNP", title = "Forest Plot of MR Results for Individual SNPs for SBP and AF") +
  theme_minimal() +
...
funnel_plot <- ggplot(SBP_harmonised_data, aes(x = beta.outcome, y = 1 / se.outcome)) +
  geom_point(alpha = 0.5, color = "blue") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs(x = expression("Genetic association with outcome " ~ beta[IV]), 
       y = expression("Inverse Standard Error " ~ 1 / SE[IV]), 
       title = "Funnel Plot to Inspect Horizontal Pleiotropy for SBP and AF") +
  theme_minimal() +
...
scatter_plot <- mr_scatter_plot(SBP_mr_results, SBP_harmonised_data)
...
head(SBP_harmonised_data)

# Save the harmonized data to a CSV file in the specified directory
output_file <- "/Users/sanjeedahs/Desktop/NEWMR/SBP/SBP_harmonized_SNPs.csv"
